...
def _extract_json_braces(text: str) -> str | None:
    ...
    if start == -1:
        return None
    depth = 0
    for i, c, in_str in _iter_string_aware(text, start):
        if in_str:
            continue
        if c == "{":
            depth += 1
...
def extract_xml(text: str | None, tag: str) -> str | None:
    ...
    if match:
        ...
        if tag == "code":
            if not _is_plausible_java(content):
                return None
        return content
    return None
...
def extract_json(text: str | None, model: type[T]) -> T:
    ...
    json_str = _replace_python_keywords(json_str)

    # Proactively remove trailing commas before closing braces/brackets
    cleaned_json = _remove_trailing_commas(json_str)
...
def _remove_trailing_commas(json_str: str) -> str:
    ...
    result = []
    skip_until = -1
    for i, c, in_str in _iter_string_aware(json_str):
        if i <= skip_until:
            continue
        if in_str:
            result.append(c)
...
def parse_guaranteed(text: str | None, tag: str, model: type[T] | None = None) -> Any:
    ...
    # 1. Try XML
    xml_content = extract_xml(text, tag)
    if xml_content:
        if model:
            try:
                return extract_json(xml_content, model)
            except Exception:
