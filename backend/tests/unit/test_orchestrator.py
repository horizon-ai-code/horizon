...
@pytest.mark.asyncio
class TestNotify:
    async def test_sends_status(self, model_config, prompt_config):  # TC-OR-005
        client = AsyncMock()
        ...
        db = MagicMock()
        validator = MagicMock()
        orch = Orchestrator(agent, validator, db, config=model_config)
        orch.prompts = prompt_config
        await orch._notify(client, Role.Planner, "test message", 2)
        assert client.send_status.await_count >= 1

    async def test_includes_model_names(self, model_config, prompt_config):  # TC-OR-006
        client = AsyncMock()
        client.id = "test-session"
        client.is_stale = False
        client.send_status = AsyncMock()
    ...
    async def test_skips_send_when_stale(self, model_config, prompt_config):  # TC-OR-007
        client = AsyncMock()
        client.send_status = AsyncMock()
