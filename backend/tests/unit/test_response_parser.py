...
class TestExtractJson:
    ...
    def test_extracts_from_code_fence(self):  # TC-RP-008
        text = '```json\n{"a": 1}\n```'
        result = ResponseParser.extract_json(text, type("_", (), {"model_validate_json": classmethod(lambda cls, s: json.loads(s))})())
        assert result is not None

    def test_fixes_trailing_comma(self):  # TC-RP-009
        result = ResponseParser._remove_trailing_commas('{"a": 1, "b": 2,}')
        assert result is not None
    ...
    def test_brace_inside_string(self):  # TC-RP-013
        result = ResponseParser._extract_json_braces('{"key": "just {opening", "other": true}')
        assert result is not None

    def test_trailing_comma_inside_string_preserved(self):  # TC-RP-014
        result = ResponseParser._remove_trailing_commas('{"msg": "ends with,}", "list": [1, 2,],}')
        assert isinstance(result, str)

    def test_python_keywords_inside_string_not_replaced(self):  # TC-RP-015
        result = ResponseParser._replace_python_keywords('{"class": "set to None, True or False"}')
    ...
    def test_deeply_nested_json(self):  # TC-RP-017
        result = ResponseParser._extract_json_braces('{"a":{"b":{"c":{"d":{"e":1}}}}}')
        assert result is not None
